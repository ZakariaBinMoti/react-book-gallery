...
const ReadBookCard = ({ book }) => {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 my-6 border border-gray-300 p-5 rounded-2xl">
      <div className="p-6 bg-[#F3F3F3] rounded-2xl flex items-center justify-center">
        <img className="h-[172px] w-[129px]" src={image} alt="" />
      </div>
      <div className="space-y-3">
        <h3 className="text-2xl playfare">{bookName}</h3>
        <p>By: {author}</p>
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          <p className="bg-[#dcebff] text-[#328EFF] rounded-full p-2">
            Category: {category}
          </p>
          <p className="text-[#FFAC33] bg-[#fff6ea] rounded-full p-2">
            Rating: {rating}
          </p>
          <Link
            className="bg-[#23BE0A] p-2 rounded-full text-white"
            to={`/book/${bookId}`}
          >
            View Details
          </Link>
